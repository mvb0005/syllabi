...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "role", sa.Enum("student", "instructor", "admin", name="userrole"), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "grading_type",
            sa.Enum("deterministic", "llm_rubric", "hybrid", name="gradingtype"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "grading", "graded", "error", name="submissionstatus"),
            nullable=False,
        ),
    ...
    op.create_index(
        op.f("ix_test_cases_assignment_id"), "test_cases", ["assignment_id"], unique=False
    )
    op.create_table(
        "grade_records",
        sa.Column("id", sa.String(length=36), nullable=False),
        ...
        sa.Column(
            "graded_by",
            sa.Enum("deterministic", "llm", "instructor_override", name="gradedby"),
            nullable=False,
        ),
    ...
    # No explicit index — the UniqueConstraint above already creates a unique
    # index on submission_id; a second non-unique index would be redundant.
...
def downgrade() -> None:
    ...
    # Must be dropped after all dependent tables have been removed.
    op.execute(sa.text("DROP TYPE IF EXISTS userrole"))
    op.execute(sa.text("DROP TYPE IF EXISTS gradingtype"))
    op.execute(sa.text("DROP TYPE IF EXISTS submissionstatus"))
    op.execute(sa.text("DROP TYPE IF EXISTS gradedby"))
    # ### end Alembic commands ###
